...
let processMsg = {
    chat: (msg, player) => {
        ...
        broadcastInRoom(player.room, {chat: msg.chat.slice(0,100)});
    },
    connected: (msg) => {
...
wss.on("connection", ws=>{
	...
	ws.on("message",(data)=>{
        ...
        for(let key in msg){
            if(processMsg[key]){
                processMsg[key](msg, players[clientId]);// hash table faster :D
            } else {
                console.log(`Server sided response for ${key} isnt defined! go to index.js and define it rn.`);
            }
        }
	})
	ws.on('close',() => {
